...
public class PropertiesNotifierTest {
	...
	public void containsAllDefaults() {
		...
		properties.setProperty(OTHER_PROPERTY, OTHER_VALUE);
		Set<String> keys = properties.stringPropertyNames();
		@SuppressWarnings("rawtypes")
		Predicate<Set> allMatch = keys::equals;
		assertTrue( allMatch.test(notifier.stringPropertyNames()) );
		assertEquals( notifier.getProperty(PROPERTY), VALUE );
		assertEquals( notifier.getProperty(OTHER_PROPERTY), OTHER_VALUE );
	}
	...
	public void notifiesWhenAnyPropertyChanges() {
		...
		notifier.addPropertyChangeListener(event -> { result[0]++; } );
	...
	public void notifiesOnlyWhenSpecificPropertyChanges() {
		int[] result = new int[]{0};
		notifier.addPropertyChangeListener(PROPERTY, event -> { result[0]++; } );
	...
	@Test
	public void notifiesOnLoadFromInputStream() {}
	
	@Test
	public void notifiesOnLoadFromReader() {}
	
	@Test
	public void notifiesOnLoadFromXML() {}
}
